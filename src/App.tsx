...
export default function App() {
  ...
  if (showSplash) {
    return (
      <div className="fixed inset-0 bg-indigo-600 flex flex-col items-center justify-center z-[100]">
        ...
        <h1 className="mt-6 text-white font-display font-bold text-3xl tracking-tight">
          MerchMagic
        </h1>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-m3-surface text-m3-on-surface">
                  ...
                  <div className="p-6 m3-card space-y-4">
                    ...
                    {!user.isPro && (
                      <div className="w-full h-3 bg-m3-surface-variant rounded-full overflow-hidden">
                        <div className="h-full bg-m3-primary" style={{ width: `${(Math.max(0, user.freeLimit - user.generationsUsed) / user.freeLimit) * 100}%` }} />
                      </div>
                    )}
      ...
      {/* Error Toast */}
      {error && (
        <div className="fixed bottom-24 left-6 right-6 bg-red-600 text-white p-4 rounded-2xl shadow-xl z-[60] flex items-center justify-between">
